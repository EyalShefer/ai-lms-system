...
/**
 * Patterns that should trigger a warning but not blocking
 */
...
export function sanitizeInput(
  ...
  if (sanitized.length > maxLength) {
    ...
    wasModified = true;
    logger.warn('LLM Security: Content truncated', {
  ...
  for (const pattern of SUSPICIOUS_PATTERNS) {
    if (pattern.test(sanitized)) {
      const matches = sanitized.match(pattern);
      if (matches) {
        logger.info('LLM Security: Suspicious pattern detected', {
  ...
  // 4. Escape our delimiter sequences
  ...
  if (detectedThreats.length > 0) {
    ...
    logger.warn('LLM Security: Potential injection detected', {
...
export function validateOutput(
  ...
  if (issues.length > 0) {
    logger.warn('LLM Security: Output validation issues', { issues });
  }
...
/**
 * Get a security summary for logging/monitoring
 */
export function getSecuritySummary(content: string): {
  length: number;
  riskLevel: string;
  threatCount: number;
} {
