...
// אתחול הקליינט של ג'מיני - שימוש ב-2.0 Flash
...
export const generateCoursePlan = async (
  ...
  const textPrompt = `
    ...
    ${fileData ? "NOTE: A document has been provided. Base the syllabus STRICTLY on the content of this document." : ""}
...
// --- פונקציה 9: Wizdi Magic - שאלה פתוחה ---
...
// --- פונקציה 10: Wizdi Magic - שאלה אמריקאית ---
...
// --- פונקציה 11 (חדשה): יצירת הצעות לפרסונות צ'אט בוט ---
export const generateChatbotPersonas = async (unitContent: string, unitTitle: string) => {
  const prompt = `
      Analyze the following learning content about "${unitTitle}":
      "${unitContent.substring(0, 2000)}..."
  
      Suggest 3 distinct chatbot personas for a student interacting with this content.
      1. "The Tutor": Supportive, clear, explains simply (Default).
      2. "The Socratic": Answers with questions, encourages critical thinking.
      3. "The Roleplay": A specific character or expert relevant to the TOPIC (e.g. if history -> Napoleon, if science -> Einstein or a Lab Assistant).
  
      Return ONLY valid JSON array:
      [
        { "id": "tutor", "name": "המורה התומך", "description": "עונה בסבלנות, מסביר מושגים ומעודד.", "systemPrompt": "You are a supportive tutor. Explain clearly..." },
        { "id": "socratic", "name": "המדריך הסוקרטי", "description": "לא נותן תשובות ישרות, אלא שואל שאלות מכוונות.", "systemPrompt": "You are a Socratic tutor. Never give the answer directly..." },
        { "id": "role", "name": "[שם הדמות הרלוונטית]", "description": "[תיאור הדמות]", "systemPrompt": "Act as [Character]..." }
      ]
      Language: Hebrew (for name and description), English (for systemPrompt).
    `;
  ...
  try {
    ...
    return [
      { id: "tutor", name: "המורה התומך (ברירת מחדל)", description: "עונה בסבלנות ומסביר את החומר.", systemPrompt: "You are a helpful tutor." },
      { id: "socratic", name: "המדריך הסוקרטי", description: "מאתגר בשאלות חשיבה.", systemPrompt: "You are a Socratic guide." }
    ];
  }
};