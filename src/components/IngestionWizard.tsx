...
const IngestionWizard: React.FC<IngestionWizardProps> = ({ onComplete, onCancel }) => {
    ...
    const handleNext = async () => {
        if (step === 1 && mode) { setStep(2); }
            ...
            setTimeout(() => { onComplete({ mode, file, topic, settings: { grade, subject, modulesCount, taxonomy } }); }, 1500);
        }
    };
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
            <div className="bg-white/90 glass w-full max-w-5xl rounded-3xl shadow-2xl overflow-hidden border border-white/50 flex flex-col max-h-[90vh] relative">
                                ...
                                <div {...getRootProps()} onClick={() => setMode('upload')} className={`group relative p-8 rounded-3xl border-2 border-dashed transition-all duration-300 text-right flex flex-col justify-between overflow-hidden h-64 cursor-pointer ${isDragActive ? 'border-blue-500 bg-blue-100' : mode === 'upload' ? 'border-blue-500 bg-blue-50/50 shadow-lg ring-4 ring-blue-100' : 'border-gray-300 bg-white hover:border-blue-400 hover:bg-gray-50'}`}>
                                    ...
                                    {file ? (
                                        <div className="flex flex-col items-center justify-center h-full text-center animate-fade-in">
                                            <div className="bg-green-100 w-20 h-20 rounded-full flex items-center justify-center mb-4 shadow-sm"><IconCheck className="w-10 h-10 text-green-600" /></div>
                                            <h4 className="text-xl font-bold text-gray-800">{file.name}</h4>
                                            <p className="text-sm text-gray-500 mt-2">הקובץ מוכן לעיבוד</p>
                                            <span className="text-xs text-blue-600 underline mt-2">לחץ להחלפה</span>
                                        </div>
                                    ) : (
                                        <>
                                            <div className="bg-purple-100 w-16 h-16 rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform"><IconUpload className="w-8 h-8 text-purple-600" /></div>
                                            <div><h4 className="text-xl font-bold text-gray-800 mb-2">העלאת קובץ לימוד</h4><p className="text-sm text-gray-500 leading-relaxed">לחץ כאן או גרור קובץ PDF/Word כדי לנתח אותו.</p></div>
                                            {mode === 'upload' && !file && <div className="absolute top-4 left-4 bg-gray-200 text-gray-500 p-1 rounded-full"><IconUpload className="w-4 h-4" /></div>}
                                        </>
                                    )}
                                </div>
                            </div>
                            {mode === 'topic' && (<div className="animate-fade-in mt-6"><label className="block text-lg font-bold text-gray-700 mb-2">באיזה נושא נעסוק היום?</label><input type="text" value={topic} onChange={(e) => setTopic(e.target.value)} className="w-full p-4 text-lg border border-gray-300 rounded-xl focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition-all shadow-sm" placeholder="למשל: המהפכה הצרפתית..." autoFocus /></div>)}
                        </div>
                    )}
                    ...
                    {step === 2 && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 animate-slide-up h-full">
                            <div className="space-y-6">
                                ...
                                <p className="text-sm text-gray-500 mb-6">קבע את תמהיל השאלות. הסכום תמיד 100%.</p>
                                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
                                    <div className="flex justify-between mb-3"><label className="font-bold text-green-600 text-lg">ידע והבנה</label><span className="font-mono font-bold text-lg bg-green-50 px-2 rounded">{taxonomy.knowledge}%</span></div>
                                    <input type="range" min="0" max="100" value={taxonomy.knowledge} onChange={(e) => handleTaxonomyChange('knowledge', parseInt(e.target.value))} className="w-full accent-green-500 h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer" />
                                </div>
                                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
                                    <div className="flex justify-between mb-3"><label className="font-bold text-blue-600 text-lg">יישום וניתוח</label><span className="font-mono font-bold text-lg bg-blue-50 px-2 rounded">{taxonomy.application}%</span></div>
                                    <input type="range" min="0" max="100" value={taxonomy.application} onChange={(e) => handleTaxonomyChange('application', parseInt(e.target.value))} className="w-full accent-blue-500 h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer" />
                                </div>
                                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
                                    <div className="flex justify-between mb-3"><label className="font-bold text-orange-600 text-lg">הערכה ויצירה</label><span className="font-mono font-bold text-lg bg-orange-50 px-2 rounded">{taxonomy.evaluation}%</span></div>
                                    <input type="range" min="0" max="100" value={taxonomy.evaluation} onChange={(e) => handleTaxonomyChange('evaluation', parseInt(e.target.value))} className="w-full accent-orange-500 h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer" />
                                </div>
                            </div>
                ...
                <div className="p-6 border-t border-gray-100 bg-gray-50/80 flex justify-between shrink-0 items-center">
                    {step === 2 && <button onClick={() => setStep(1)} className="text-gray-500 hover:text-blue-600 font-bold px-4 flex items-center gap-2"><IconArrowBack className="w-4 h-4" /> חזרה</button>}
                    <button
                        onClick={handleNext}
