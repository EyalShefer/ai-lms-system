...
// הגדרת אייקונים מקומיים כדי למנוע קריסה אם הם חסרים בקובץ האייקונים הראשי
...
const UnitEditor: React.FC<UnitEditorProps> = ({ unit, gradeLevel = "כללי", onSave, onCancel, cancelLabel = "חזרה" }) => {
    ...
    const generatePedagogicalPrompt = (persona: string, customCharName: string = ""): string => {
        const baseInfo = `אתה עוזר הוראה עבור תלמידים בשכבת גיל: ${gradeLevel}. הנושא הנלמד: "${editedUnit.title}".`;
        const safetyProtocol = `פרוטוקול בטיחות (חובה): 1. שמור על שפה מכבדת. 2. אם התלמיד מביע מצוקה או אלימות - הפסק את השיח הלימודי והפנה אותו למבוגר.`;
        ...
        if (showScoring) {
            ...
            let personaGuidance = "";
            switch (persona) {
                case 'teacher': personaGuidance = "הייה מורה סבלני ומעודד."; break;
            ...
            specificInstructions = `מצב למידה: ${personaGuidance}. אל תיתן תשובה מיד, אלא רמזים תחילה.`;
        }
        ...
        const newBlock: ActivityBlock = {
            ...
            content: type === 'multiple-choice' ? { question: '', options: ['', '', '', ''], correctAnswer: '' }
                : type === 'open-question' ? { question: '' }
                    : type === 'interactive-chat' ? { title: showScoring ? 'עזרה במבחן' : 'דיאלוג עם המורה', description: 'צאט...' }
                        : '',
            metadata: {
                score: 0,
            ...
            <div>
                {activeInsertIndex === index ? (
                    ...
                    <button onClick={() => setActiveInsertIndex(index)} className="bg-white text-blue-600 border border-blue-200 rounded-full px-4 py-1.5 flex items-center gap-2 shadow-sm hover:shadow-md hover:scale-105 transition-all hover:bg-blue-50 hover:border-blue-300">
                        <IconPlus className="w-4 h-4" /><span className="text-xs font-bold">הוסף רכיב</span>
                    </button>
                )}
            </div>
        </div>
        ...
        <div className="min-h-screen p-8 font-sans pb-24 bg-gray-50/50">
            ...
            <div className="sticky top-4 z-40 glass backdrop-blur-lg shadow-lg rounded-2xl p-4 flex justify-between items-center mb-10 border border-white/60 bg-white/80 gap-4">
                <div className="flex-1 min-w-0">
                    <input type="text" value={editedUnit.title} onChange={(e) => setEditedUnit({ ...editedUnit, title: e.target.value })} className="text-2xl font-bold text-gray-800 bg-transparent border-b border-transparent focus:border-blue-500 outline-none px-2 transition-colors placeholder-gray-400 w-full" placeholder="כותרת היחידה" />
                    <div className="text-sm text-gray-500 px-2 mt-1 flex items-center gap-2">
                        ...
                        <span className={`font-bold ${showScoring ? 'text-blue-800' : 'text-green-700'}`}>{showScoring ? 'מצב מבחן' : 'מצב למידה'}</span>
                    </div>
                </div>
                <div className="flex gap-3 flex-shrink-0">
                    ...
                    <button onClick={onCancel} className="px-5 py-2 rounded-xl text-gray-600 hover:bg-white/50 font-medium transition-colors flex items-center gap-2"><IconBack className="w-4 h-4" /> {cancelLabel}</button>

                    {/* כפתור שמירה חכם עם חיווי ויזואלי */}
                    <button
                    ...
                    <React.Fragment key={block.id}>
                        <div className="glass p-6 rounded-2xl shadow-sm border border-white/60 hover:shadow-xl hover:border-blue-200 transition-all relative group bg-white/80">
                            ...
                            <div className="mt-2">
                                ...
                                {block.type === 'text' && (
                                    <div>
                                        <textarea className="w-full p-4 border border-gray-200/60 bg-white/50 rounded-xl focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all text-gray-700 leading-relaxed resize-y min-h-[120px]" value={block.content || ''} onChange={(e) => updateBlock(block.id, e.target.value)} placeholder="הקלד כאן את תוכן הלימוד..." />
                                        ...
                                        <div className="flex flex-wrap items-center gap-2 mt-3 bg-blue-50/40 p-2 rounded-xl border border-blue-100/50 backdrop-blur-sm justify-between">
                                            ...
                                            <div className="flex gap-1 border-r border-blue-200 pr-2">
                                                <label className={mediaBtnClass} title="הוסף תמונה"><IconImage className="w-4 h-4" /><input type="file" accept="image/*" className="hidden" onChange={(e) => handleFileUpload(e, block.id, 'metadata')} /></label>
                                                <label className={mediaBtnClass} title="הוסף וידאו"><IconVideo className="w-4 h-4" /><input type="file" accept="video/*" className="hidden" onChange={(e) => handleFileUpload(e, block.id, 'metadata')} /></label>
                                            </div>
                                        </div>
                                    </div>
                                )}
                                    ...
                                    <div className="bg-gradient-to-r from-indigo-50/80 to-blue-50/80 p-5 rounded-xl border border-indigo-100 backdrop-blur-sm">
                                        ...
                                        {showScoring ? (
                                            ...
                                            <div className="flex items-center gap-2 bg-white/60 p-3 rounded-lg text-indigo-800 mb-4 border border-indigo-100 shadow-sm">
                                                ...
                                                <div className="flex-1">
                                                    <span className="font-bold text-sm block">מצב למידה פעיל</span>
                                                    <span className="text-xs text-indigo-600">הבוט מוגדר כמורה מלווה ומסייע בלמידה.</span>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        <div className="flex items-center gap-3 mb-4">
                                            <div className="flex-1">
                                                <label className="text-xs font-bold text-indigo-700 block mb-1">כותרת הבוט (מוצגת לתלמיד)</label>
                                                <input type="text" className="w-full p-2.5 border border-indigo-200 rounded-lg bg-white/70 focus:bg-white transition-colors text-gray-800 font-medium" value={block.content.title || ''} onChange={(e) => updateBlock(block.id, { ...block.content, title: e.target.value })} placeholder="למשל: המורה המלווה שלך" />
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                            <div className={showScoring ? 'opacity-50 pointer-events-none grayscale' : ''}>
                                                ...
                                                <select className="w-full p-2.5 border border-indigo-200 rounded-lg bg-white/70 focus:bg-white transition-colors cursor-pointer text-sm" value={block.metadata?.botPersona || 'teacher'} onChange={(e) => handlePersonaChange(block.id, e.target.value, block.content, block.metadata)} disabled={showScoring}>
                                                    <option value="teacher">מורה פרטי</option>
                                                    <option value="socratic">מורה סוקרטי</option>
                                        ...
                                        <div>
                                            <label className="text-xs font-bold text-indigo-700 block mb-1">הודעת פתיחה לתלמיד</label>
                                    ...
                                    <div className={`${block.type === 'multiple-choice' ? 'bg-sky-50/40 border-sky-100' : 'bg-teal-50/40 border-teal-100'} p-5 rounded-xl border backdrop-blur-sm`}>
                                        ...
                                        {block.type === 'open-question' && (
                                            <div className="pr-7 mt-2">
                                                ...
                                                <textarea className="w-full p-3 border border-gray-200/80 rounded-xl bg-white/80 text-sm focus:bg-white transition-colors outline-none focus:border-teal-300" rows={2} value={block.metadata?.modelAnswer || ''} onChange={(e) => updateBlock(block.id, block.content, { modelAnswer: e.target.value })} placeholder="כתוב כאן את התשובה המצופה..." />
                                            </div>
                                        )}
