            ...
            <div className="sticky top-4 z-40 glass backdrop-blur-lg shadow-lg rounded-2xl p-4 flex justify-between items-center mb-10 border border-white/60 bg-white/80">
                <div>
                    <input
                        ...
                        className="text-2xl font-bold text-gray-800 bg-transparent border-b border-transparent focus:border-blue-500 outline-none px-2 transition-colors placeholder-gray-400"
                ...
                <div className="flex gap-3">
                    ...
                    <button onClick={onCancel} className="px-5 py-2 rounded-xl text-gray-600 hover:bg-white/50 font-medium transition-colors flex items-center gap-2">
                        <IconBack className="w-4 h-4" /> ביטול
                    </button>
            ...
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                {editedUnit.activityBlocks?.map((block, index) => (
                    <React.Fragment key={block.id}>
                                ...
                                {block.type === 'image' && (
                                    <div>
                                        ...
                                        <div className="bg-blue-50/50 p-3 rounded-xl border border-blue-100 mt-2 flex gap-2 items-center backdrop-blur-sm">
                                            ...
                                            <button onClick={() => handleSuggestImagePrompt(block.id)} disabled={loadingBlockId === block.id} className="bg-white text-blue-600 border border-blue-100 px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-blue-50">
                                                <IconWand className="w-3 h-3 inline ml-1" /> הצע
                                            </button>
                                            <button onClick={() => handlePaintImage(block.id, block.metadata?.aiPrompt || '')} disabled={loadingBlockId === block.id} className="bg-blue-600 text-white px-4 py-1.5 rounded-lg text-xs font-bold shadow-md hover:bg-blue-700 transition-transform hover:scale-105">
                                                צייר!
                                            </button>
                                        </div>
                                    </div>
                                ...
                                {block.type === 'video' && (
                                    <div>
                                        ...
                                        <div className="mt-2">
                                            <textarea
                                                className="w-full p-3 text-sm border border-gray-100 rounded-xl bg-white/50 focus:bg-white outline-none transition-all placeholder-gray-400"
                                                placeholder="הוסף כיתוב או הסבר לסרטון..."
                                                value={block.metadata?.caption || ''}
                                                onChange={(e) => updateBlock(block.id, block.content, { caption: e.target.value })}
                                                rows={2}
                                            />
                                ...
                                {block.type === 'interactive-chat' && (
                                    <div className="bg-gradient-to-r from-purple-50/80 to-indigo-50/80 p-5 rounded-xl border border-purple-100 backdrop-blur-sm">
                                        ...
                                        <div className="space-y-4">
                                            <div>
                                                <label className="text-xs font-bold text-purple-700 block mb-1">כותרת הפעילות</label>
                                                <input type="text" className="w-full p-2.5 border border-purple-200 rounded-lg bg-white/70 focus:bg-white transition-colors" value={block.content.title || ''} onChange={(e) => updateBlock(block.id, { ...block.content, title: e.target.value })} placeholder="למשל: שיחה עם דמות היסטורית" />
                                            </div>
                                            <div>
                                                <label className="text-xs font-bold text-purple-700 block mb-1">הנחיה ל-AI (System Prompt) - מי הדמות?</label>
                                                <textarea className="w-full p-2.5 border border-purple-200 rounded-lg bg-white/70 h-24 text-sm focus:bg-white transition-colors resize-none" value={block.metadata?.systemPrompt || ''} onChange={(e) => updateBlock(block.id, block.content, { systemPrompt: e.target.value })} placeholder='למשל: "אתה הרצל. ענה לתלמידים בשפה גבוהה והדגש את הציונות."' />
                                            </div>
                                            <div>
                                                <label className="text-xs font-bold text-purple-700 block mb-1">הודעת פתיחה לתלמיד</label>
                                                <input type="text" className="w-full p-2.5 border border-purple-200 rounded-lg bg-white/70 text-sm focus:bg-white transition-colors" value={block.metadata?.initialMessage || ''} onChange={(e) => updateBlock(block.id, block.content, { initialMessage: e.target.value })} placeholder='למשל: "שלום, שאל אותי כל דבר על אלטנוילנד."' />
                                            </div>
                                        </div>
                                    </div>
                                )}

                                {/* QUESTIONS (MULTIPLE CHOICE & OPEN) */}
                                {(block.type === 'multiple-choice' || block.type === 'open-question') && (
                                    <div className={`${block.type === 'multiple-choice' ? 'bg-blue-50/40 border-blue-100' : 'bg-orange-50/40 border-orange-100'} p-5 rounded-xl border backdrop-blur-sm`}>
                                        <div className="flex justify-between items-start mb-4">
                                            ...
                                            {showScoring && (
                                                <div className="flex flex-col items-center ml-2 bg-white/50 p-1 rounded-lg border border-gray-100">
                                                    <span className="text-[10px] font-bold text-gray-400 uppercase">ניקוד</span>
                                                    <input type="number" className="w-12 text-center p-1 rounded border-none bg-transparent text-sm font-bold" value={block.metadata?.score || 0} onChange={(e) => updateBlock(block.id, block.content, { score: Number(e.target.value) })} />
                                                </div>
                                            )}
                                        </div>

                                        {/* תצוגת מדיה (תמונה/וידאו) בתוך השאלה */}
                                        {renderEmbeddedMedia(block.id, block.metadata)}

                                        {block.type === 'multiple-choice' && (
                                            <div className="space-y-2 pr-7">
                                                {/* כפתור קסם AI - מופיע רק אם השאלה ריקה */}
                                                {!block.content.question && (
                                                    <div className="flex justify-end mb-4">
                                                        <button
                                                            onClick={() => handleAutoGenerateMCQuestion(block.id)}
                                                            disabled={loadingBlockId === block.id}
                                                            className="bg-gradient-to-r from-blue-500 to-indigo-500 text-white px-4 py-1.5 rounded-full text-xs font-bold shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all flex items-center gap-2"
                                                        >
                                                            {loadingBlockId === block.id ? 'חושב...' : <><IconSparkles className="w-3 h-3" /> Wizdi Magic: צור שאלה אמריקאית</>}
                                                        </button>
                                                    </div>
                                                )}
                                                ...
                                                {block.content.options?.map((opt: string, idx: number) => (
                                                    <div key={idx} className="flex items-center gap-3 group/option">
                                                        <button
                                                            onClick={() => updateBlock(block.id, { ...block.content, correctAnswer: opt })}
                                                            className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${block.content.correctAnswer === opt ? 'bg-green-500 border-green-500 text-white shadow-sm scale-110' : 'bg-white border-gray-300 hover:border-gray-400'}`}
                                                        >
                                                            {block.content.correctAnswer === opt && <IconCheck className="w-3.5 h-3.5" />}
                                                        </button>
                                                        <input
                                                            type="text"
                                                            className={`flex-1 p-2 text-sm border rounded-lg bg-white/80 focus:bg-white focus:ring-2 outline-none transition-all ${block.content.correctAnswer === opt ? 'border-green-200 ring-green-50' : 'border-gray-200 focus:ring-blue-50 focus:border-blue-300'}`}
                                                            value={opt}
                                                            onChange={(e) => { const newOptions = [...block.content.options]; newOptions[idx] = e.target.value; updateBlock(block.id, { ...block.content, options: newOptions }); }}
                                            ...
                                            <div className="pr-7 mt-2">
                                                {!block.content.question && (
                                                    <div className="flex justify-end mb-2">
                                                        <button
                                                            onClick={() => handleAutoGenerateOpenQuestion(block.id)}
                                                            disabled={loadingBlockId === block.id}
                                        ...
                                        {/* --- סרגל כלים פנימי להוספת מדיה בתוך השאלה --- */}
                                        <div className="mt-3 pt-3 border-t border-gray-200/50 flex justify-between items-center">
                                            ...
                                            <div className="flex gap-2">
                                                <label className={mediaBtnClass} title="הוסף תמונה">
                                                    <IconImage className="w-4 h-4" /> תמונה
                                                    <input type="file" accept="image/*" className="hidden" onChange={(e) => handleFileUpload(e, block.id, 'metadata')} />
                                                </label>
                                                <label className={mediaBtnClass} title="הוסף וידאו">
                                                    <IconVideo className="w-4 h-4" /> וידאו
                                                    <input type="file" accept="video/*" className="hidden" onChange={(e) => handleFileUpload(e, block.id, 'metadata')} />
                                                </label>
                                            </div>
                                        </div>
                        ...
                        {/* תפריט הוספה חיצוני - מופיע רק כשלוחצים עליו */}
