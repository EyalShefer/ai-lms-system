...
function App() {
  const [mode, setMode] = useState<'editor' | 'student' | 'dashboard'>('editor');
      ...
      <header className="bg-white shadow-sm p-4 flex justify-between items-center sticky top-0 z-50 border-b border-gray-200">
        ...
        <div className="bg-gray-100 p-1 rounded-lg flex text-sm font-bold">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-md transition-all flex items-center gap-2 ${mode === 'student' ? 'bg-white shadow text-green-600' : 'text-gray-500 hover:text-gray-700'
              }`}
          >
            <span>👨‍🎓</span> תלמיד
          </button>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-md transition-all flex items-center gap-2 ${mode === 'dashboard' ? 'bg-white shadow text-purple-600' : 'text-gray-500 hover:text-gray-700'
              }`}
          >
            <span>📊</span> דשבורד
          </button>
